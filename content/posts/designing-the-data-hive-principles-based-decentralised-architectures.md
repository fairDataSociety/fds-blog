...
The key to achieving the ideal is detailed in the Swarm whitepaper \[6\]. The overarching vision is born through the deployment of the technology stack, designed to act as a medium through which the Swarm network optimises network bandwidth and storage resources (hard disk memory). It consists of a P2P network of nodes that communicate according to an agreed distributed consensus mechanism and leveraged by an in-built incentivisation scheme.
...
Swarm is not the first incarnation of a blockchain-based decentralised storage infrastructure \[7\], with notable projects such as Sia \[8\], Filecoin \[9\], and Storj \[10\] also deployed into the market. Distributed content storage \[11\] also has a rich history, developing alongside distributed file systems such as the open source project, InterPlanetary File System (IPFS) \[12\]. What sets Swarm apart is the explicit commitment to a defined set of open data principles, along with a commitment to building a fair data economy.
